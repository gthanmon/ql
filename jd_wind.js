...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      let totalBean = await TotalBean2();
      let jingxiang = totalBean.uclass;
      jingxiang = jingxiang.replace("京享值", "")

      let pg_channel_page = await Pg_channel_page();

      message = `【京东账号${$.index}】\n`;
      message += `【京享值】${jingxiang}\n`;
      if(pg_channel_page && pg_channel_page.changeScore) {
        message += `【京享值变动】${pg_channel_page.changeScore}\n`;
      }

      message += `【等级】${level}\n`;
      message += `【综合分1】${totalScore}\n`;
      message += `【综合分2】信用:${baiScore} 购物合规:${shop} 购物历史:${shopAfter} 售后行为:${active} 账户信息:${accountInfo}\n`;
